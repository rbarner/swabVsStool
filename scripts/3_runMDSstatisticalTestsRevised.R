...
mdsStatisticalModels <- function(dataSet,eigen,tool,level)
{
  ...
  for(i in 1:8)
  {
    ...
    pValIndividualList[[length(pValIndividualList)+1]]<- pValParticipant;
  }
  ...
  names(makeTable)=cbind("Origin adj p-value","Participant adj p-value");
  write("MDS Axis\t% variation explained\tOrigin adj p-value\tParticipant adj p-value",paste("../statisticalModels/3_mds_",taxa,"_",classifier,"_individual_origin_pVal.txt",sep=""));
...
for(classifier in classifierList)
{
  ...
  for(taxa in taxaLevels )
  {
    ...
    mdsStatisticalModels(mdsMeta,eigen,classifier,taxa);
    setwd("..") 
  }
}
...
for(funct in functionList)
{
  ...
  for(wgs in wgsLevels )
  {
    ...
    eigen <-readRDS(eigenFile);
    mdsStatisticalModels(mdsMeta,eigen,funct,wgs);
    setwd("..")
  }
}