...
for(taxa in taxaLevels )
{
  ...
  setwd("../statisticalModels/")
  write("Axis\t% explained\tOrigin p-value\tTime p-value\tIndividual p-value",paste("4_anovaMDSindividual_",taxa,"_metaphlan.txt",sep=""));
  write.table(makeTable,paste("4_anovaMDSindividual_",taxa,"_metaphlan.txt",sep=""),quote=FALSE, sep="\t",append=TRUE, col.names=FALSE);
  
  setwd("../plots")
  
  tiff(paste("4_mdsPvalues_LineGraph_",taxa,"_metaphlan.tiff",sep=""),width=200,height=200,units="mm",compression="lzw",res=350);
  maxLimit  <- summary(unlist(-log10(makeTable)))[6]+3;
  minLimit  <- summary(unlist(-log10(makeTable)))[1]-2;
...
for(wgs in wgsLevels )
{
  ...
  {     
    ...
    simpleMod <- gls(model,method="REML",data=mdsMeta2);
    mixedMod <- lme(model,method="REML",random=~1|study_id,data=mdsMeta);
